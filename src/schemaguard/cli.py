...
from schemaguard import migration_analyzer, model_analyzer, schema_file_analyzer, reporter, scanner

def build_parser():
    p = argparse.ArgumentParser(prog="schemaguard", description="Detect breaking schema changes before they reach production.")
    ...
    p.add_argument("--format", choices=("text","json","markdown","github","sarif"), default="text")
    p.add_argument("--fail-on", choices=("none","low","medium","high","critical"), default="high", dest="fail_on")
...
def main():
    ...
    if args.format == "json": print(reporter.format_json(report))
    elif args.format == "markdown": print(reporter.format_markdown(report))
    elif args.format == "github": print(reporter.format_github(report))
    elif args.format == "sarif": print(reporter.format_sarif(report))
    else: print(reporter.format_text(report))
    
    risk_order = ["none","low","medium","high","critical"]
    if risk_order.index(str(report["risk_level"])) >= risk_order.index(args.fail_on) and args.fail_on != "none":
        sys.exit(1)
